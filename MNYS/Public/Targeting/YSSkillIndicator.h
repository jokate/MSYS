...
/**
 * 플레이어가 스킬을 레디했을 때 띄우는 조준 인디케이터입니다.
 *
 * AYSTelegraphActor(적 공격 전조)와 목적이 다릅니다.
 * 전조는 서버가 월드에 배치하고 수명이 끝나면 사라지는 "여기에 맞는다"는 경고이고,
 * 이쪽은 로컬 플레이어의 입력을 따라 매 프레임 움직이는 "여기에 쓸 것이다"는 예고입니다.
 * 그래서 복제하지 않고 충돌도 갖지 않습니다.
 *
 * 스스로 Tick 하지 않습니다. 조준 지점 산출과 갱신 호출은 전부 타게팅 쪽 책임이고,
 * 이 액터는 넘겨받은 값을 그리기만 합니다.
 *
 * 동시에 레디되는 스킬은 항상 1개이므로 인스턴스를 하나 만들어 두고 숨겼다 켜는 방식으로 씁니다.
 * 스킬을 레디할 때마다 스폰/디스트로이 하지 마십시오.
 *
 * 머티리얼 UV 규약 — 데칼은 아래로 투영되며, 머티리얼은 정사각 UV(0,0)~(1,1)을 받습니다.
 *   Circle : 중심이 (0.5, 0.5)
 *   Cone   : 꼭짓점이 한 변의 중앙, 거기서 반대편으로 벌어짐
 *   Line   : 시작점이 한 변의 중앙, 거기서 반대편으로 뻗음
 */
